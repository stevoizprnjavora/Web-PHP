...
--
-- Table structure for table `users555`
--

CREATE TABLE `users555` (
  `id` int(11) NOT NULL,
  `firstname` varchar(255) NOT NULL,
  `lastname` varchar(255) NOT NULL,
  `email` varchar(255) NOT NULL,
...
--
-- Dumping data for table `users555`
--

INSERT INTO `users555` (`id`, `firstname`, `lastname`, `email`, `username`, `password`, `country`, `date`, `archive`) VALUES
...
--
-- Structure for view `n`
--
DROP TABLE IF EXISTS `n`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `n`  AS SELECT `users`.`user_id` AS `user_id`, `users`.`user_name` AS `user_name`, `users`.`password` AS `password` FROM `users` ;
...
--
-- Indexes for table `users555`
--
ALTER TABLE `users555`
  ADD PRIMARY KEY (`id`);
...
--
-- AUTO_INCREMENT for table `users555`
--
ALTER TABLE `users555`
